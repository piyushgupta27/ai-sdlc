...
- **a prior internal pattern doc (private)** — Piyush's authored canonical pattern doc at `~/Workspace/local-vault/projects/active/internal-project/docs/jira-to-pr-creator-execution-plan-v2.md`
...
Already defined in [HITL.md](./HITL.md) §"Tier ↔ gate matrix" and per-project CLAUDE.md. Shared vocab with prior pattern doc:
...
### G1. Tier 0 — extreme caution (three-layer enforcement, prior pattern doc §7.1)
...
> **Parent reference:** prior pattern doc v2 Appendix A "CLAUDE.md template" is the structural source. Fields adapted per project stack.
...
### 9.2 CONTEXT.md template (adapted from prior pattern doc Appendix A)
...
### 9.4 Bubble-up rule (prior pattern doc §A.3)
...
### 10.2 Trust contraction protocol (prior pattern doc §7.2 — autonomy reduces on incidents)
...
> **Parent reference:** prior pattern doc v2 §7.2.

### 11.1 Formal expansion criteria (prior pattern doc §7.2, all required)
...
### 12.2 Smart model routing (prior pattern doc §5.4)
...
### 12.3 Prompt caching policy (prior pattern doc §10.2 — ~22% cost reduction)
