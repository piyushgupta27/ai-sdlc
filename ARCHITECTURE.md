...
Why two tiers map to Red: slice's Red zone collapses what we split into Tier 0 ("never autonomous, ever") vs Tier 1 ("never autonomous without human at COMMIT"). The split lets us run Tier 1 work through PLAN + BUILD + REVIEW + DEMO while still gating the actual merge.
...
**Red zone never reclassifies downward.** Slice plan calls it a "permanent architectural decision, not a trust signal." Adopted verbatim.
...
Per-ticket cost savings: $0.91 → $0.71 (~22% at slice scale; less at our scale due to smaller CONTEXT tree).
